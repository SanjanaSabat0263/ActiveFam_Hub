...
router.post('/addcalorieintake', authTokenHandler, async (req, res) => {
    ...
    request.get({
        ...
        if (error) return console.error('Request failed:', error);
            ...
            await user.save();
            res.json(createResponse(true, 'Calorie intake added successfully', user.calorieIntake[user.calorieIntake.length - 1]));
        }
    });
...
router.post('/getcalorieintakebydate', authTokenHandler, async (req, res) => {
    ...
    if (!date) {
        let date = new Date();   // feb 1 2026 12:00:00
...
router.put('/updatecalorieintake/:id', authTokenHandler, async (req, res) => {
    const { id } = req.params;
    const { item, date, quantity, quantitytype } = req.body;
    if (!item || !date || !quantity || !quantitytype) {
        return res.status(400).json(createResponse(false, 'Please provide all the details'));
    ...
    request.get({
        ...
        if (error) return console.error('Request failed:', error);
            ...
            const userId = req.userId;
            const user = await User.findOne({ _id: userId });
            const entryIndex = user.calorieIntake.findIndex(entry => entry.id === id);
            if (entryIndex === -1) {
                return res.status(404).json(createResponse(false, 'Calorie intake entry not found'));
            }
            user.calorieIntake[entryIndex] = {
                id,
                item,
                date: new Date(date),
                quantity,
            ...
            await user.save();
            res.json(createResponse(true, 'Calorie intake updated successfully', user.calorieIntake[entryIndex]));
        }
    });
});

router.delete('/deletecalorieintake/:id', authTokenHandler, async (req, res) => {
    const { id } = req.params;
...
router.get('/getallcalorieintake', authTokenHandler, async (req, res) => {
    const userId = req.userId;
    const user = await User.findById({ _id: userId });

    res.json(createResponse(true, 'All calorie intake entries', user.calorieIntake));
});
